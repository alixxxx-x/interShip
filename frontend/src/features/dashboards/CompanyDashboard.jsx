              ...
              <div className="flex items-end justify-between mt-2 sm:mt-2.5 md:mt-3 lg:mt-2.5 xl:mt-4 2xl:mt-5">
                ...
                <div className="flex gap-0.5 sm:gap-1 mb-0.5 flex-shrink-0">
                  {Array.from({ length: 4 }).map((_, i) => {
                    const filledCount = Math.round(((stats?.recruitmentScore ?? 100) / 100) * 4);
                    ...
                    return isFilled ? (
                      ...
                      <div key={i} className="h-2.5 w-2.5 sm:h-3 sm:w-3 md:h-3.5 md:w-3.5 lg:h-2.5 lg:w-2.5 xl:h-3.5 xl:w-3.5 2xl:h-4 2xl:w-4 rounded-full bg-muted flex items-center justify-center border border-border" />
                    );
                  })}
                </div>
              </div>
