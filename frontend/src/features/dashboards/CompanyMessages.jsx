...
export default function CompanyMessages() {
  ...
  // Fetch students who applied to this company's internships
  const fetchConversations = async () => {
    try {
      // We fetch applications to get the list of students
      ...
      // 2. Fetch admins
      const adminRes = await api.get("/users/?role=ADMIN_DEPT,ADMIN_UNIV");
      const admins = adminRes.data.results || adminRes.data;

      const adminContacts = admins.map(admin => ({
        id: admin.id,
        name: `${admin.role === 'ADMIN_DEPT' ? 'Dept Admin' : 'Univ Admin'} (${admin.username || admin.email})`,
        email: admin.email,
        department: admin.role === 'ADMIN_DEPT' ? "Department Administrator" : "University Administrator",
        isAdmin: true
      }));

      setConversations([...adminContacts, ...uniqueStudents]);
      checkUnreadStatus();
    } catch (error) {
  ...
  useEffect(() => {
    if (activeRecipient) {
      ...
      ws.current.onmessage = (event) => {
        ...
        // If we are currently viewing the chat and we received a message, we mark it as read on the server by default (or the next GET will do it)
  ...
  const handleSendMessage = async (e) => {
    ...
    if (ws.current && ws.current.readyState === WebSocket.OPEN) {
      ...
      setNewMessage("");
      setReplyingTo(null);
      // Check status slightly after send
        ...
        <Card className="w-80 flex flex-col border-none shadow-xl bg-card/50 backdrop-blur-sm overflow-hidden">
          ...
          <div className="flex-1 overflow-y-auto p-2 space-y-1">
            {filteredConversations.map((student) => (
              <button
                key={student.id}
                onClick={() => setActiveRecipient(student)}
                className={cn(
              ...
              >
                <div className={cn(
                  ...
                  activeRecipient?.id === student.id ? "bg-white/20 border-white/20" : "bg-primary/10 border-primary/20"
                )}>
                  {student.isAdmin ? (
                    <Building2 className={cn("h-5 w-5", activeRecipient?.id === student.id ? "text-white" : "text-primary")} />
                  ) : (
                    <UserIcon className={cn("h-5 w-5", activeRecipient?.id === student.id ? "text-white" : "text-primary")} />
                  )}
                </div>
                <div className="flex-1 overflow-hidden">
                  ...
                  <p className={cn(
                    "text-[10px] truncate",
                    activeRecipient?.id === student.id ? "text-white/70" : "text-muted-foreground"
                  )}>
                    {student.department}
                  </p>
                </div>
          ...
          {activeRecipient ? (
            ...
            <div className="flex-1 flex flex-col items-center justify-center text-muted-foreground gap-6 p-12 text-center">
              ...
              <div className="space-y-2">
                <h3 className="text-xl font-bold text-foreground">Select a Candidate</h3>
                <p className="max-w-[300px]">Choose a student from the left sidebar to start direct communication.</p>
              </div>
            </div>
          )}
        </Card>
