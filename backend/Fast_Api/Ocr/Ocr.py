...
async def parse_cv_with_gemini(file_bytes: bytes, mime_type: str):
    ...
    try:
        ...
        print(f"RAW AI RESPONSE: {response.text}") # For debugging in terminal
...
async def upload_cv(
    ...
    try:
        extracted_data = await parse_cv_with_gemini(file_bytes, file.content_type)
    except Exception as e:
